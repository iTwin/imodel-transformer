...
export function validateComparisonReport(report: ComparisonReport): void {
  ...
  const expectedAggregate = aggregateLogRatios(
    report.collapsedObservations.map((pair) => pair.logRatio)
  );
  if (canonicalJson(report.aggregate) !== canonicalJson(expectedAggregate))
    throw new Error(
      "Comparison report aggregate does not match its collapsed observations"
    );
  if (
