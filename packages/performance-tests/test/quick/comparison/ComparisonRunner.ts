...
import type { PreparedDetachedDataset } from "../FixtureMaterializer";
import { createChangesetScanningScenario } from "../scenarios/changesetScanning";
import {
  ArmRuntimeIdentity,
  ArmSpec,
...
import { classifyEnvironment, EnvironmentClass } from "./EnvironmentClass";
import {
...
import { deriveNoiseBand, NoiseBand, NoiseBandPool } from "./NoiseBand";
...
export async function runPair(options: {
  ...
  const launcher =
    ...
    (async (request, timeout) =>
      spawnArmProcess(
        options.armProcessPath ?? path.join(__dirname, "armProcessCli.js"),
        request,
        timeout
      ));
