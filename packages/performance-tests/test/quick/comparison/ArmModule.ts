...
import type { ChangedInstanceIdsDependency } from "../scenarios/changesetScanning";
...
export function aliasHarnessCoreBackendToArm(arm: ResolvedArmSpec): void {
  ...
  const harnessEntry = require.resolve("@itwin/core-backend");
  armRequire("@itwin/core-backend");
  const armModule = require.cache[armEntry];
  if (!armModule)
    throw new Error(
      `Arm "${arm.spec.id}" core-backend module could not be initialized`
    );
  require.cache[harnessEntry] = armModule;
}
