...
const profName = (profileName: string) => {
  ...
  return path.join(
    profileDir,
    `${profileName}${nameTimePortion}${profileExtension}`
  );
};

export default function RunWithSqliteProfiler(
  ...
  for (let i = 0; i < funcData.length; ++i) {
    ...
    object[key] = function (...args: any[]) {
      const db = argOffset
        ...
        : args.find(
            (a): a is IModelDb =>
              typeof a?.nativeDb?.startProfiler === "function"
          );
      if (db === undefined)
        throw Error(
          "no argument of the instrumented function was an IModelDb to profile"
        );
