...
/** Stateful hub behavior required by the transformer changeset tests. */
export class TransformerTestHub implements BackendHubAccess {
  ...
  public get iTwinId(): GuidString {
    if (!this._iTwinId) throw new Error("TransformerTestHub is not active");
    return this._iTwinId;
  }

  public start(name: string, outputDir: string): void {
    if (this.isActive) throw new Error("TransformerTestHub is already active");

    this._rootDir = path.join(outputDir, "TransformerTestHub", name);
  ...
  public stop(): void {
    ...
    if (errors.length > 0)
      throw new AggregateError(errors, "Failed to stop TransformerTestHub");
  }

  private get _rootDirName(): string {
    if (!this._rootDir) throw new Error("TransformerTestHub is not active");
...
export const transformerTestHub = new TransformerTestHub();
