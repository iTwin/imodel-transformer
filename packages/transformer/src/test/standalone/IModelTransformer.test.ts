...
describe("IModelTransformer", () => {
  ...
  it("save reverse sync version for processAll", async () => {
    ...
    const transformer1 = new IModelTransformer(sourceDb, targetDb);
    ...
    const reverseSyncVersion1 = scopingEsa1?.jsonProperties.reverseSyncVersion;
    ...
    const transformer2 = new IModelTransformer(sourceDb, targetDb);
    ...
    const reverseSyncVersion2 = scopingEsa2?.jsonProperties.reverseSyncVersion;
    assert.isNotEmpty(reverseSyncVersion2);
    const expectedReverseSyncVersion1 = `${targetDb.changeset.id};${targetDb.changeset.index}`;
    assert.equal(reverseSyncVersion2, expectedReverseSyncVersion1);

    const sourceModelId4 = PhysicalModel.insert(
      sourceDb,
      IModel.rootSubjectId,
      "M4"
    );
    assert.isDefined(sourceModelId4);

    // adding saveReverseVersion to true - we expect to save reverse sync version
    const transformer3 = new IModelTransformer(sourceDb, targetDb);
    ...
    const reverseSyncVersion3 = scopingEsa3?.jsonProperties.reverseSyncVersion;
    assert.isNotEmpty(reverseSyncVersion3);
    const expectedReverseSyncVersion2 = `${targetDb.changeset.id};${targetDb.changeset.index}`;
