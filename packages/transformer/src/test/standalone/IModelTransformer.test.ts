...
describe("IModelTransformer", () => {
  ...
  it.only("should be able to switch from old relationship provenance method to new relationship provenance method", async () => {
    const sourceDbFile: string = IModelTransformerTestUtils.prepareOutputFile(
      "IModelTransformer",
      "OldToNewRelationshipProvenance.bim"
    );
    const sourceDb = SnapshotDb.createEmpty(sourceDbFile, {
      rootSubject: { name: "OldToNewRelationship" },
    });
    ...
    const targetDbFile = IModelTransformerTestUtils.prepareOutputFile(
      "IModelTransformer",
      "OldToNewRelationshipProvenanceTarget.bim"
    );
    const targetDb = SnapshotDb.createEmpty(targetDbFile, {
      rootSubject: { name: "Combined Model" },
    });
    ...
    let relIdInTarget = transformer.context
      .findTargetEntityId(`r${relId}`)
      .slice(1);
    let aspects: ExternalSourceAspect[] = targetDb.elements.getAspects(
      sourceIdInTarget,
      ExternalSourceAspect.classFullName
    ...
    expect(
      JSON.parse(aspects[0].jsonProperties ?? "").targetRelInstanceId
    ).to.equal(relIdInTarget);
    // expect().to.equal()
    // Previously the transformer would insert provenance always pointing to the "target" relationship.
    // * It should (and now by default does) instead insert provenance pointing to the provenanceSource
    ...
    aspects = targetDb.elements.getAspects(
      sourceIdInTarget,
      ExternalSourceAspect.classFullName
    ) as ExternalSourceAspect[];
